...
from .evaluation.qa_eval import QAExactMatch, QAF1Score, calculate_overall_qa_metrics
...
class HippoRAG:
    ...
    def initialize_graph(self):
        ...
        self._graph_pickle_filename = os.path.join(
            self.embedding_dir, f"graph.pickle"
        )
    ...
    def retrieve(self,
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            # Save intermediate facts before and after rerank to a single file
            intermediate_dir = os.path.join(self.working_dir, "intermediate_results")
            os.makedirs(intermediate_dir, exist_ok=True)
            rerank_file = os.path.join(intermediate_dir, f"rerank_facts_q{q_idx}.json")
            with open(rerank_file, 'w', encoding='utf-8') as f:
                json.dump({
                    "facts_before_rerank": rerank_log["facts_before_rerank"],
                    "facts_after_rerank": rerank_log["facts_after_rerank"]
    ...
    def rag_qa(self,
        ...
        if gold_answers is not None:
            ...
            logger.info(f"Evaluation results for HippoRAG QA: {overall_qa_results}")
    ...
    def rag_qa_dpr(self,
        ...
        if gold_answers is not None:
            ...
            logger.info(f"Evaluation results for HippoRAG DPR QA: {overall_qa_results}")
    ...
    def qa(self, queries: List[QuerySolution], gold_answers: Optional[List[List[str]]] = None) -> Tuple[List[QuerySolution], List[str], List[Dict]]:
        ...
        for query_solution_idx, query_solution in tqdm(enumerate(queries), desc="Extraction Answers from LLM Response"):
            ...
            # Merge facts and QA messages into one intermediate JSON
            import os
            import json
            intermediate_dir = os.path.join(self.working_dir, "intermediate_results")
            os.makedirs(intermediate_dir, exist_ok=True)
            facts_file = os.path.join(intermediate_dir, f"rerank_facts_q{query_solution_idx}.json")
            try:
                with open(facts_file, 'r', encoding='utf-8') as ff:
                    facts_data = json.load(ff)
            except Exception:
                facts_data = {"facts_before_rerank": [], "facts_after_rerank": []}
            merged_filepath = os.path.join(intermediate_dir, f"intermediate_q{query_solution_idx}.json")
            with open(merged_filepath, 'w', encoding='utf-8') as mf:
                json.dump({
                    "facts_before_rerank": facts_data.get("facts_before_rerank", []),
                    "facts_after_rerank": facts_data.get("facts_after_rerank", []),
                    "qa": {
                        "messages": all_qa_messages[query_solution_idx],
                        "response": response_content,
                        "metadata": all_metadata[query_solution_idx]
                    }
                }, mf, indent=2, ensure_ascii=False)

            # Save the final response including merged intermediate reference
            self.save_final_response_with_intermediates(
                query_solution.question, response_content, query_solution_idx, {"intermediate": merged_filepath}, query_gold_answers
            )
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        phrase_weights[mask] /= number_of_occurs[mask]
