...
class BaseConfig:
    ...
    exclude_llm_from_paths: bool = field(
        default=False,
        metadata={"help": "If set to True, will exclude LLM name from embedding and OpenIE paths for reusability."}
    )
    embedding_base_dir: str = field(
        default=None,
        metadata={"help": "Base directory for universal embeddings and OpenIE when exclude_llm_from_paths is True."}
    )
    llm_cache_dir: str = field(
        default=None,
        metadata={"help": "Directory for LLM cache files."}
    )
    ...
    recall_eval_top_k: int = field(
        default=200,
        metadata={"help": "Retrieving k documents at each step for recall evaluation"}
    )
