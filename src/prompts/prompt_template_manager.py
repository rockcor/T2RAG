...
class PromptTemplateManager:
    ...
    def _load_templates(self) -> None:
        ...
        Now supports subdirectories (e.g., cgrag/).
        """
        ...
        self._load_templates_recursive(self.templates_dir, "")

    def _load_templates_recursive(self, directory: str, prefix: str) -> None:
        """
        Recursively load templates from a directory and its subdirectories.
        
        Args:
            directory (str): The directory to scan
            prefix (str): The prefix to add to template names (for subdirectories)
        """
        for item in os.listdir(directory):
            ...
            if os.path.isdir(item_path) and not item.startswith('__'):
                ...
                subdir_prefix = f"{prefix}{item}/" if prefix else f"{item}/"
                self._load_templates_recursive(item_path, subdir_prefix)
                
            elif item.endswith(".py") and item != "__init__.py":
                script_name = os.path.splitext(item)[0]
                template_name = f"{prefix}{script_name}" if prefix else script_name

                try:
                    # Build the module name for subdirectories
                    if prefix:
                        # Replace / with . for module path
                        module_path = prefix.replace('/', '.') + script_name
                        try:
                            module_name = f"src.prompts.templates.{module_path}"
                            module = importlib.import_module(module_name)
                        except ModuleNotFoundError:
                            module_name = f".prompts.templates.{module_path}"
                            module = importlib.import_module(module_name, 'hipporag')
                    else:
                        # Original logic for root templates
                        try:
                            module_name = f"src.prompts.templates.{script_name}"
                            module = importlib.import_module(module_name)
                        except ModuleNotFoundError:
                    ...
                    if isinstance(prompt_template, Template):
                        ...
                        self.templates[template_name] = Template(prompt_template)
                    ...
                    logger.debug(f"Successfully loaded template '{template_name}' from '{module_name}.py'.")
